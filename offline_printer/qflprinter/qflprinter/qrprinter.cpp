...
string ReturnCurrTagsReceipt(vector<NameTag> const &tag);
...
string ReturnCurrTagsReceipt(vector<NameTag> const &tag) {
	string str = "";
	for (int i = 0; i < sizeof(tag) / sizeof(NameTag); i++) {
		str += (tag[i].ID //22 refs
			+ "," + tag[i].partyID
			+ "," + tag[i].churchNameENG
			+ "," + tag[i].contactName
			+ "," + tag[i].partyType
...
void PrintReceipt(char* strCpy, int partyID) {
	//constant values
	const char* data = "./qrres/reporttemplate3.csv"; //input datasheet for all customers
	...
	while (getline(fheader, line, '\n')) {
		if (line.find("KEYWORD: HEADER END") != string::npos) //loop until program finds this commented keyword to denote end of css and formatting
			break;
		if (line.find("PARTY_FIELD") != string::npos)
			fillField(line, "PARTY_FIELD", tag.partyID); //fill in Party ID Field
		dataout << line << '\n';
	}
	...
	for (NameTag currTag : tags) {
		...
		fillField(modtemplate, "ID_FIELD", currTag.ID); //fill in ID Field
		fillField(modtemplate, "NAMEENG_FIELD", currTag.fname + " " + currTag.lname); //fill in Individual Name Field
		fillField(modtemplate, "ROOM_FIELD", currTag.room); //fill in Room name
		fillField(modtemplate, "KEYSTATUS_FIELD", "Y"); //fill in Key Status Field
		fillField(modtemplate, "CARDSTATUS_FIELD", "N"); //fill in Card Field
	...
	fheader.close();
	dataout.close();
	ReturnCurrTagsReceipt(tags);
	//string str = ReturnCurrTagsReceipt(tags);
